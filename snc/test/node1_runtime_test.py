#!/usr/bin/env python3
"""
Runtime tests for the navigation node (Node #1 - Navigation Logic).
This script monitors the topic activity of the navigation node
and validates if the node is performing as expected based on the spec.

The test monitors:
- /map topic (nav_msgs/msg/OccupancyGrid) - map published by SLAM
- /hazards topic (visualization_msgs/msg/Marker) - hazard markers
...
class NavigationRuntimeTest(Node):
    def __init__(self):
        super().__init__('navigation_runtime_test')
        ...
        # Subscribe to map topic (nav_msgs/msg/OccupancyGrid)
        ...
        self.get_logger().info('Navigation runtime test initialized')
        self.get_logger().info('Subscribed to topics:')
        self.get_logger().info('  - /map (nav_msgs/msg/OccupancyGrid)')
    ...
    def map_callback(self, msg):
        ...
        if self.last_map_time is not None:
            time_diff = time.time() - self.last_map_time
            if time_diff > 0:
                self.map_publishing_rate = 1.0 / time_diff
        ...
        width = msg.info.width if hasattr(msg, 'info') and hasattr(msg.info, 'width') else 0
        height = msg.info.height if hasattr(msg, 'info') and hasattr(msg.info, 'height') else 0
        
        self.get_logger().info(
            f'Received map #{self.map_count} '
            f'(Frame: {frame_id}, Size: {width}x{height}, Resolution: {msg.info.resolution:.3f})'
        )
    ...
    def validate_map(self):
        """Validate that the map is being published correctly"""
        self.get_logger().info('Validating map...')
        
        if self.map_count > 0:
            self.get_logger().info(f'✓ Maps received: {self.map_count}')
            
            if self.latest_map:
                frame_id = self.latest_map.header.frame_id if hasattr(self.latest_map, 'header') else 'unknown'
                width = self.latest_map.info.width if hasattr(self.latest_map, 'info') else 0
                height = self.latest_map.info.height if hasattr(self.latest_map, 'info') else 0
                resolution = self.latest_map.info.resolution if hasattr(self.latest_map, 'info') else 0.0
                
                self.get_logger().info(f'  Latest map frame: {frame_id}')
                self.get_logger().info(f'  Latest map size: {width}x{height}')
                self.get_logger().info(f'  Latest map resolution: {resolution:.3f}')
                
                if frame_id == 'map':
                    self.get_logger().info('  ✓ Map frame is correct (map)')
                else:
                    self.get_logger().error(f'  ✗ Map frame incorrect: {frame_id}')
                    return False
            else:
                self.get_logger().info('  ? No map data available')
        else:
            self.get_logger().info('ℹ No maps received yet')
    ...
    def validate_hazard_markers(self):
        ...
        if self.hazard_marker_count > 0:
            ...
            if self.latest_hazard_marker:
                frame_id = self.latest_hazard_marker.header.frame_id if hasattr(self.latest_hazard_marker, 'header') else 'unknown'
                self.get_logger().info(f'  Latest hazard marker frame: {frame_id}')
                self.get_logger().info(f'  Latest hazard marker ID: {self.latest_hazard_marker.id}')
                
                if frame_id == 'map':
                    self.get_logger().info('  ✓ Hazard marker frame is correct (map)')
                else:
                    self.get_logger().error(f'  ✗ Hazard marker frame incorrect: {frame_id}')
                    return False
            else:
    ...
    def validate_status(self):
        """Validate that status messages are being published"""
        self.get_logger().info('Validating status messages...')
        ...
        if self.status_message_count > 0:
            ...
            if self.latest_status:
                ...
                self.get_logger().info('  ? No status data available')
        else:
    ...
    def validate_triggers(self):
        """Validate trigger topic activity"""
    ...
    def run_test(self):
        ...
        self.get_logger().info(f'Trigger teleop received: {self.trigger_teleop_count} times')
        self.get_logger().info(f'Trigger home received: {self.trigger_home_count} times')
        self.get_logger().info(f'Test duration: indefinite (until interrupted)')
...
def main():
    ...
    # Create test node
    test_node = NavigationRuntimeTest()
