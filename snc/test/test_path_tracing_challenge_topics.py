...
class PathTracingRuntimeTest(Node):
    def __init__(self):
        ...
        self.log_file_path = os.path.join(self.log_dir, f'path_tracing_challenge_topics_{timestamp}.log')
        self.summary_file_path = os.path.join(self.log_dir, f'path_tracing_challenge_summary_{timestamp}.log')
        ...
        self.file_logger.info('PATH TRACING RUNTIME TEST - CHALLENGE TOPICS')
        self.file_logger.info('=' * 60)
        ...
        self.get_logger().info('PATH TRACING RUNTIME TEST - CHALLENGE TOPICS')
        self.get_logger().info('=' * 60)
    ...
    def explore_path_callback(self, msg):
        ...
        if not self.first_waypoint_received and len(msg.poses) > 0:
            ...
            message = 'First path waypoint received. Starting 10-second timer for go_home...'
            self.get_logger().info(message)
            self.file_logger.info(message)
    ...
    def go_home_callback(self):
        ...
        if self.go_home_timer:
            ...
            self.go_home_timer = None
        message = '10 seconds elapsed. Publishing go_home signal...'
        self.get_logger().info(message)
        self.file_logger.info(message)
        self.pub_go_home.publish(GO_HOME_INTERFACE())
        message = f'Published to {GO_HOME_TOPIC}'
        self.get_logger().info(message)
        self.file_logger.info(message)
    ...
    def startup_sync_callback(self, msg):
        ...
        if node_name in self.startup_sync_signals:
            ...
            if set(self.startup_sync_signals.keys()) == self.expected_nodes:
                ...
                message = 'Publishing snc_start to trigger challenge...'
                self.get_logger().info(message)
                self.file_logger.info(message)
                self.pub_start_challenge.publish(START_CHALLENGE_INTERFACE())
                message = f'Published to {START_CHALLENGE_TOPIC}'
                self.get_logger().info(message)
                self.file_logger.info(message)
