#!/usr/bin/env python3
"""
Alternate version of the path tracing runtime test.
This script waits for the synchronization check to complete,
publishes the trigger start topic, and then publishes the trigger teleop topic.
"""
...
class PathTracingRuntimeTest(Node):
    def __init__(self):
        ...
        # Publisher for trigger_teleop topic
        ...
        # Flag to track if trigger_start has been published
        self.trigger_start_published = False
        ...
        self.log_file_path = os.path.join(self.log_dir, f'path_tracing_trigger_teleop_{timestamp}.log')
        self.summary_file_path = os.path.join(self.log_dir, f'path_tracing_trigger_teleop_summary_{timestamp}.log')
        ...
        self.file_logger.info('PATH TRACING RUNTIME TEST (ALTERNATE) - INITIALIZING')
        self.file_logger.info('=' * 60)
        ...
        self.file_logger.info(f'  - {TRIGGER_TELEOP_TOPIC}')
        ...
        self.get_logger().info(f'  - {TRIGGER_TELEOP_TOPIC}')
    ...
    def startup_sync_callback(self, msg):
        ...
        if node_name in self.startup_sync_signals:
            ...
            if set(self.startup_sync_signals.keys()) == self.expected_nodes:
                ...
                self.get_logger().info(message)
                self.file_logger.info(message)
                self.trigger_start_published = True
                
                # Small delay before publishing trigger_teleop
                message = 'Waiting 1 second before publishing trigger_teleop...'
                self.get_logger().info(message)
                self.file_logger.info(message)
                time.sleep(1.0)
                
                # Publish trigger_teleop to enable teleop control
                message = 'Publishing trigger_teleop to enable teleop control...'
                self.get_logger().info(message)
                self.file_logger.info(message)
                self.pub_trigger_teleop.publish(TRIGGER_TELEOP_INTERFACE())
                message = f'Published to {TRIGGER_TELEOP_TOPIC}'
                self.get_logger().info(message)
                self.file_logger.info(message)
                self.trigger_teleop_published = True
    ...
    def run_test(self):
        ...
        with open(self.summary_file_path, 'w') as f:
            ...
            f.write(f'Trigger start published: {self.trigger_start_published}\n')
            f.write(f'Trigger teleop published: {self.trigger_teleop_published}\n')
        ...
        self.get_logger().info(f'Trigger start published: {self.trigger_start_published}')
        self.get_logger().info(f'Trigger teleop published: {self.trigger_teleop_published}')
