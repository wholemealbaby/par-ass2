# COSC2781/COSC2814 ROSBot and AIIL Workspace Setup (2026)

This is TheConstruct examples and setup with the Husarion ROSBot and AIIL standard code in ROS2 Jazzy.
...
Thus when the ROSJect launches again, it will run the script ``~/course_install.sh``. This can take quite some time.

**Before** running any commands for the first time when opening this ROSJect you need to:

1. Wait for the course install script to finish
1. Re-build colcon in the `aiil_workspace/humble_workspace` folder
```
...
* Empty world - an empty world that only spawns a ROSbot
    - ``ros2 launch rosbot_gazebo simulation.yaml robot_model:=rosbot rviz:=False gz_world:=/home/user/aiil_workspace/humble_workspace/src/husarion/husarion_office_gz/worlds/empty_with_plugins.sdf``
...
* Maze - an enclosed maze developed for AIIL assignments
    - ``ros2 launch rosbot_gazebo simulation.yaml robot_model:=rosbot rviz:=False gz_world:=/home/user/aiil_workspace/humble_workspace/src/aiil_gazebo/worlds/maze.sdf``
* Maze with Hazard signs - the enclosed maze with Hazard signs scattered around the maze. This will be useful for the Search & Navigation challenge.
    - ``ros2 launch rosbot_gazebo simulation.yaml robot_model:=rosbot rviz:=False gz_world:=/home/user/aiil_workspace/humble_workspace/src/aiil_gazebo/worlds/hazards.sdf``
...
* Navigation - uses the configuration slam files in aiil_gazebo which may differ slightly from the ROSBot
    - ``ros2 launch aiil_gazebo nav_bringup.launch.py``
...
1. Clone your repository into the ``humble_workspace/src`` folder. For example (you can replace the ``my_packges`` folder name as you desire:
    ```
    git clone <url> ~/aiil_workspace/humble_workspace/src/par_coursework/my_packages
    ```
    This cloned repo repository will serve as the root folder in which you will place all your packages. Colcon (and ROS) allows sub-folder layouts. That is, your folder structure should be very similar to the VM setup and ROSBot setup.
